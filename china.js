...
function ready(error, china, provinces) {
...
function makeMap(china, provinces) {
    ...
    var center = d3.geoCentroid(china);
    var ratio = 500 / 960;
    ...
    var pop = d3.extent(chinadata, function (d) {
        return +d.properties.Population
    });
    var res = d3.extent(chinadata, function (d) {
        return +d.properties.Residents
    });
    // setup x
    var xScale = d3.scaleLinear().range([100, width - 20]).domain(pop); // value -> display
        var xAxis = d3.axisBottom(xScale)
        .ticks(5);

    // setup y
    var yScale = d3.scaleLinear().range([height - 100, 60]).domain(res); // value -> display
